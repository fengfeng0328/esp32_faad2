...
//	}
    ...
    /* Set configuration */
    config = NeAACDecGetCurrentConfiguration(hDecoder);	// 返回解码器库配置
    config->outputFormat = outputFormat;
    config->downMatrix = downMatrix;
    //config->dontUpSampleImplicitSBR = 1;
    NeAACDecSetConfiguration(hDecoder, config);			// 设置解码库配置
...
//		printf("frameInfo.samples:\t%ld\n", frameInfo.samples);
//		printf("frameInfo.channels:\t%d\n", frameInfo.channels);
//		printf("dur:\t\t\t%ld\n", dur);
//		printf("decoded\t\t\t%d\n", decoded);

//		printf("mp4config.samples:\t%d\n", mp4config.samples);
        if (decoded > mp4config.samples)	/* 探究mp4config.samples的由来 */
...
/* System dependant types */
    ...
    /* 此函数以32位无符号整数返回解码器的能力。32位无符号整数中设置的位定义了编译库的能力 */
    ...
    printf("Read Byte:\t\t%s\n","8 Byte");
    fread(header, 1, 8, hMP4File);	// read 8 Byte, front 4 char is length, Behind 4 char is ftyp

	if (!readFromStdin) {
		printf("!readFromStdin = 1\thMP4File fclose\n");
		fclose(hMP4File);
	}
...
//		printf("length:\t\t\t%f\n", length);
    ...
    if (audioFileName != NULL)
      free (audioFileName);		// remind free
    if (adtsFileName != NULL)
